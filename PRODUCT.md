...
The long-term shape is **Master (control) vs Child (project execution)**. Mac app and mobile web are Studio clients; **Hermes** (e.g. WhatsApp) is an external commander that can drive both Master and project/Child work. Project execution may later run on a dedicated **workstation node** (including Windows 11), with **always-on** daemon and optional **Cloudflare Zero Trust** remote URL (e.g. `agent.madebyaris.com`) for phone control while away. See [ADR 008](./docs/decisions/008-master-child-hermes-workstation.md), [ADR 011](./docs/decisions/011-always-on-node.md), [ADR 012](./docs/decisions/012-cloudflare-zero-trust-remote.md).
